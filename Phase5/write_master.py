def write_master_bank_accounts(accounts, file_path):
    ...
    """
    # Sort accounts by account_number as integer
    ...
    with open(file_path, 'w') as file:
        for acc in accounts:
            ...
            status = acc['status']
            balance = f"{acc['balance']:08.2f}"  # Always 8 characters
            txns = str(acc['total_transactions']).zfill(4)

            # Combine fields into exactly 42 characters
            line = f"{acc_num} {name} {status} {balance} {txns}"

            if len(line) != 42:
                raise ValueError(f"Formatted line is not 42 characters: {line} (len={len(line)})")
